...
define("PROJECT_ROOT", '/var/www');
define("APP_ROOT", PROJECT_ROOT . '/html');
...
foreach ([
  ...
  if (empty($env_value = getenv($env_key))) {
    throw new RuntimeException(
      sprintf(
        'Expected the environment variable %s, but it is either empty or not declared.  Cannot continue.',
        $env_key
      )
    );
  }
 ...
 */
// Database connection.
// Use values declared in container environment.
$databases['default']['default'] = [
  'database' => getenv('MYSQL_HOST'),
...
$config_directories['sync'] = CONFIG_DIRECTORY;
...
if (getenv('APP_ENV') === 'dev' && file_exists(
    __DIR__ . '/settings.ddev.php'
  )) {
